...
const shareCheckIn = () => {
  const title = '我的签到记录'
  const content = `坚持训练第${stats.value.currentStreak}天！累计签到${stats.value.totalDays}天，本月已签到${stats.value.thisMonthCount}天。\n\n${isTodayCheckedIn.value ? `今日已通过${getTodayGameType()}游戏完成签到` : '今日尚未签到，完成任意训练即可签到'}。\n\n每日训练，持续精进！一起来挑战舒尔特专注训练，提升认知能力吧！`
  
  // 检查是否在App WebView中，如果是则使用JSBridge分享
  if (window.SchulteNative && typeof window.SchulteNative.shareContent === 'function') {
    try {
      const success = window.SchulteNative.shareContent(title, content)
      if (success) {
        console.log('使用JSBridge分享成功')
        return
      } else {
        console.warn('JSBridge分享失败，使用Web方式')
      }
    } catch (error) {
  ...
  // 降级到Web分享API或剪贴板
  if (navigator.share) {
    navigator.share({ title, text: content }).catch(error => {
